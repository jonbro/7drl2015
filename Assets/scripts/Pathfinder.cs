...
namespace RL{
	...
	public class Pathfinder{
		SortedDictionary<UInt64, Node> openList;
		SortedDictionary<UInt64, Node> closedList;
		int[] OriginalSearchOrder = new int[4]{0,1,2,3};
		int[] CurrentSearchOrder = new int[4]{0,1,2,3};

		public void SetSearchOrder(int[] searchOrder = null){
			if (searchOrder == null)
				CurrentSearchOrder = OriginalSearchOrder;
			else
		...
		public List<Vector2i> FindPath(Vector2i start, Vector2i end, CostCallback costFn, Map map){
			...
			// clear the lists
			openList = new SortedDictionary<ulong, Node> ();
			closedList = new SortedDictionary<ulong, Node> ();
			...
			while (!pathfound && openList.Count > 0 && searchCount < 500) {
				...
				for (int i = 0; i < 4; i++) {
					...
					Vector2i neighborPosition = new Vector2i (lowestCost.position.x + Map.nDir [CurrentSearchOrder[i], 0], lowestCost.position.y + Map.nDir [CurrentSearchOrder[i], 1]);
